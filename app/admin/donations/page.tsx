  ...
  return (
    <>
      ...
      <p className="mb-5 text-sm text-ink-mid">
        From members and non-members alike — professors, alumni, the university,
        local businesses.
      </p>
      ...
      <div className="grid gap-5 lg:grid-cols-[1fr_1.2fr] lg:items-start">
        <DonationForm funds={fundList.map((f) => ({ id: f.id, name: f.name }))} />
        ...
        <div>
          ...
          {list.length ? (
            ...
            <Empty title="No donations yet"
              body="Record the first one with the form. Donors who are not thanked rarely give twice, so the reminder counter matters." />
          )}
        </div>
      </div>
    </>
